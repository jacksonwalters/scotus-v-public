...
def relevant_cases_by_opin_id(keywords,scotus_vocab,scotus_tfidf_matrix_sparse,scotus_opin_id):
    ...
    num_opinions = scotus_tfidf_matrix_sparse.shape[0]
    scores = []
    for row in range(num_opinions):
        score = sum(scotus_tfidf_matrix_sparse[row,ind] for ind in keyword_ind)
        """
        score = 0
